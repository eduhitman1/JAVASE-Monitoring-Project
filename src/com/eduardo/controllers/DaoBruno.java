...
public class DaoBruno {

    ConexaoBD conex = new ConexaoBD();
    BeansBruno mod = new BeansBruno();

    public void salvar(BeansBruno mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("insert into bruno( num_c, tipofer_c, tipo_c, status_c, grupo_c, sumario_c, data_l , esc_1, esc_2, esc_3, esc_4, esc_5, data_s,tipofor_c, tipoana_c, hora, data_email)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public void editar(BeansBruno mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("update bruno set num_c=?, tipofer_c=?,tipo_c=?, status_c=?,  grupo_c=?, sumario_c=?, data_l=?, esc_1=?, esc_2=?, esc_3=?, esc_4=?, esc_5=?, data_s=?, tipofor_c=?, tipoana_c=?, data_email=? where cod_c=?");
            pst.setString(1, mod.getNum_c());
            pst.setString(2, mod.getTipofer_c());
            pst.setString(3, mod.getTipo_c());
            pst.setString(4, mod.getStatus_c());
    ...
    public BeansBruno buscaChamado(BeansBruno mod) {
        conex.conexao();
        conex.executaSql("select *from bruno where num_c like'%" + mod.getPesquisa() + "%'"); // passand string sql
        try {
            conex.rs.first();
    ...
    public void excluir(BeansBruno mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("delete from bruno where cod_c=?");
