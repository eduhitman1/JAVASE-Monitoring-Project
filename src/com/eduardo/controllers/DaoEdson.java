 ...
 * @author Eduardo
 */
public class DaoEdson {

    ConexaoBD conex = new ConexaoBD();
    BeansEdson mod = new BeansEdson();

    public void salvar(BeansEdson mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("insert into edson( num_c, tipofer_c, tipo_c, status_c, grupo_c, sumario_c, data_l , esc_1, esc_2, esc_3, esc_4, esc_5, data_s,tipofor_c, tipoana_c, hora, data_email)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public void editar(BeansEdson mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("update edson set num_c=?, tipofer_c=?,tipo_c=?, status_c=?,  grupo_c=?, sumario_c=?, data_l=?, esc_1=?, esc_2=?, esc_3=?, esc_4=?, esc_5=?, data_s=?, tipofor_c=?, tipoana_c=?, data_email=? where cod_c=?");
            pst.setString(1, mod.getNum_c());
            pst.setString(2, mod.getTipofer_c());
            pst.setString(3, mod.getTipo_c());
            pst.setString(4, mod.getStatus_c());
    ...
    public BeansEdson buscaChamado(BeansEdson mod) {
        conex.conexao();
        conex.executaSql("select *from edson where num_c like'%" + mod.getPesquisa() + "%'"); // passand string sql
        try {
            conex.rs.first();
    ...
    public void excluir(BeansEdson mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("delete from edson where cod_c=?");
