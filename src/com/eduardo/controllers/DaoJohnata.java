 ...
 * @author Eduardo
 */
public class DaoJohnata {

    ConexaoBD conex = new ConexaoBD();
    BeansJohnata mod = new BeansJohnata();

    public void salvar(BeansJohnata mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("insert into johnata( num_c, tipofer_c, tipo_c, status_c, grupo_c, sumario_c, data_l , esc_1, esc_2, esc_3, esc_4, esc_5, data_s,tipofor_c, tipoana_c, hora, data_email)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public void editar(BeansJohnata mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("update johnata set num_c=?, tipofer_c=?,tipo_c=?, status_c=?,  grupo_c=?, sumario_c=?, data_l=?, esc_1=?, esc_2=?, esc_3=?, esc_4=?, esc_5=?, data_s=?, tipofor_c=?, tipoana_c=?, data_email=? where cod_c=?");
            pst.setString(1, mod.getNum_c());
            pst.setString(2, mod.getTipofer_c());
            pst.setString(3, mod.getTipo_c());
            pst.setString(4, mod.getStatus_c());
    ...
    public BeansJohnata buscaChamado(BeansJohnata mod) {
        conex.conexao();
        conex.executaSql("select *from johnata where num_c like'%" + mod.getPesquisa() + "%'"); // passand string sql
        try {
            conex.rs.first();
            ...
            mod.setData_email(conex.rs.getString("dataEmail"));
        } catch (SQLException ex) {
    ...
    public void excluir(BeansJohnata mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("delete from johnata where cod_c=?");
