 ...
 * @author Eduardo
 */
public class DaoDanileia {

    ConexaoBD conex = new ConexaoBD();
    BeansDanileia mod = new BeansDanileia();

    public void salvar(BeansDanileia mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("insert into danileia( num_c, tipofer_c, tipo_c, status_c, grupo_c, sumario_c, data_l , esc_1, esc_2, esc_3, esc_4, esc_5, data_s,tipofor_c, tipoana_c, hora, data_email)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public void editar(BeansDanileia mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("update danileia set num_c=?, tipofer_c=?,tipo_c=?, status_c=?,  grupo_c=?, sumario_c=?, data_l=?, esc_1=?, esc_2=?, esc_3=?, esc_4=?, esc_5=?, data_s=?, tipofor_c=?, tipoana_c=?, data_email=? where cod_c=?");
            pst.setString(1, mod.getNum_c());
            pst.setString(2, mod.getTipofer_c());
            pst.setString(3, mod.getTipo_c());
            pst.setString(4, mod.getStatus_c());
    ...
    public BeansDanileia buscaChamado(BeansDanileia mod) {
        conex.conexao();
        conex.executaSql("select *from danileia where num_c like'%" + mod.getPesquisa() + "%'"); // passand string sql
        try {
            conex.rs.first();
    ...
    public void excluir(BeansDanileia mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("delete from danileia where cod_c=?");
