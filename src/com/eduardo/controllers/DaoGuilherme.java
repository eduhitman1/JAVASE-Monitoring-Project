 ...
 * @author Eduardo
 */
public class DaoGuilherme {

    ConexaoBD conex = new ConexaoBD();
    BeansGuilherme mod = new BeansGuilherme();

    public void salvar(BeansGuilherme mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("insert into guilherme( num_c, tipofer_c, tipo_c, status_c, grupo_c, sumario_c, data_l , esc_1, esc_2, esc_3, esc_4, esc_5, data_s,tipofor_c, tipoana_c, hora, data_email)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            pst.setString(17, mod.getData_email());
            pst.execute();
    ...
    public void editar(BeansGuilherme mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("update guilherme set num_c=?, tipofer_c=?,tipo_c=?, status_c=?,  grupo_c=?, sumario_c=?, data_l=?, esc_1=?, esc_2=?, esc_3=?, esc_4=?, esc_5=?, data_s=?, tipofor_c=?, tipoana_c=?, data_email=? where cod_c=?");
            pst.setString(1, mod.getNum_c());
            pst.setString(2, mod.getTipofer_c());
            pst.setString(3, mod.getTipo_c());
            pst.setString(4, mod.getStatus_c());
            ...
            pst.setString(14, mod.getTipofor_c());
            pst.setString(15, mod.getTipoAna_c());
            pst.setString(16, mod.getData_email());
            pst.setInt(17, mod.getCod_c());
    ...
    public BeansGuilherme buscaChamado(BeansGuilherme mod) {
        conex.conexao();
        conex.executaSql("select *from guilherme where num_c like'%" + mod.getPesquisa() + "%'"); // passand string sql
        try {
            conex.rs.first();
            ...
            mod.setData_email(conex.rs.getString("data_email"));
        } catch (SQLException ex) {
    ...
    public void excluir(BeansGuilherme mod) {
        conex.conexao();
        try {
            PreparedStatement pst = conex.con.prepareStatement("delete from guilherme where cod_c=?");
